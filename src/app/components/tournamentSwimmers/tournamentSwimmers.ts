...
import { TournamentSwimmerRegisterController } from 'app/components/tournamentSwimmerRegister/tournamentSwimmerRegister';
...
/*@ngInject*/
 class TournamentSwimmers {
    ...
    registerSwimmers($event) {
        this.$mdDialog.show({
            controller: TournamentSwimmerRegisterController,
            controllerAs: '$ctrl',
            templateUrl: 'app/components/tournamentSwimmerRegister/tournamentSwimmerRegister.html',
            ...
            escapeToClose: true,
            locals: {
                excludeSwimmers: this.tournament.swimmers
            }
        }).then((swimmers: Array<Swimmer>) => {
            swimmers.forEach((newSwimmer) => {
                this.tournament.swimmerIds.push(newSwimmer.id);
                this.tournament.swimmers.push(newSwimmer);
            });

            this.tournamentService.updateSwimmers(this.tournament.id, this.tournament.swimmerIds);
        }, () => {
            //
        });
    }
